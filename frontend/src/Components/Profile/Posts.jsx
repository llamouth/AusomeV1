...
// COMPONENTS
import DeletePost from '../Feed/Posts/DeletePost';
import CommentsContainer from '../Feed/Posts/Comments/CommentsContainer';
...
const Posts = ({ post, setAllPosts, setRun }) => {
    ...
    const { id } = useParams();
    const newLike = { user_id: userLoggedIn };
    ...
    const handleLike = (e) => {
        ...
        if (likedPost) {
            const liked = likes.find(like => like.user_id == userLoggedIn);
    ...
    useEffect(() => {
        ...
        .then( res => res.json() )
        .then(res => {
            const liked = res.find( like => like.user_id == userLoggedIn);
